...
namespace SrcChess2.Core {
    ...
    public sealed class ChessBoard : ISearchTrace<Move>, IXmlSerializable {
        ...
        public static readonly AttackPosInfo s_attackPosInfoNull = new(0, 0, new int[16]);
        ...
        /// <summary>
        /// Check if piece on specified position is specified type
        /// </summary>
        /// <param name="x">checking position</param>
        /// <param name="piece">checking piece</param>
        /// <returns>if x is existing position on board return is piece equal to parameter otherwise false</returns>
        public bool CheckIsPiece(int x, PieceType piece)
        {
            if (x < 0 || x > 63)
                return false;
            return m_board[x] == piece;
        }
        ...
        public int CalculateKingShield(PieceType color, int pos)
        {
            int x = pos / 8, y = pos % 8;
            if ((x != 7 && x != 0) || y == 0 || y == 7)
                return 0;
            ...
            if (x == 7)
            {
                if (m_board[48 + y] == (PieceType.Pawn | color))
                    retVal++;
                if (m_board[48 + y + 1] == (PieceType.Pawn | color))
                    retVal++;
                if (m_board[48 + y - 1] == (PieceType.Pawn | color))
                    retVal++;
            }
            ...
            {
                if (m_board[8 + y] == (PieceType.Pawn | color))
                    retVal++;
                if (m_board[8 + y + 1] == (PieceType.Pawn | color))
                    retVal++;
                if (m_board[8 + y - 1] == (PieceType.Pawn | color))
                    retVal++;
            }
        ...
        public List<Move>? EnumMoveList(PlayerColor playerColor, bool isMoveListNeeded, out AttackPosInfo attackPosInfo) {
            ...
            m_posInfo.PiecesMobility = new int[16];
            m_posInfo.PiecesAttacked  = 0;
            m_posInfo.PiecesDefending = 0;
            m_posInfo.WhiteCheck = IsCheck(PlayerColor.White);
            m_posInfo.BlackCheck = IsCheck(PlayerColor.Black);
            m_posInfo.BlackKingShield = CalculateKingShield(PieceType.Black, m_blackKingPos);
            m_posInfo.WhiteKingShield = CalculateKingShield(PieceType.White, m_whiteKingPos);
            ...
            for (int i = 0; i < 64; i++)
            {
                ...
                {
                    f.Clear();
                    PlayerColor playerColor2 = (pieceType & PieceType.Black) == PieceType.Black ? PlayerColor.Black : PlayerColor.White;
                    switch (pieceType & PieceType.PieceMask)
                    {
                        case PieceType.Pawn:
                            EnumPawnMove(playerColor2, i, f);
                            break;
                        case PieceType.Knight:
                            EnumFromArray(playerColor2, i, s_caseMoveKnight[i], f);
                            break;
                        case PieceType.Bishop:
                            EnumFromArray(playerColor2, i, s_caseMoveDiagonal[i], f);
                            break;
                        case PieceType.Rook:
                            EnumFromArray(playerColor2, i, s_caseMoveLine[i], f);
                            break;
                        case PieceType.Queen:
                            EnumFromArray(playerColor2, i, s_caseMoveDiagLine[i], f);
                            break;
                        case PieceType.King:
                            EnumFromArray(playerColor2, i, s_caseMoveKing[i], f);
                            break;
                    }
                    m_posInfo.PiecesMobility[(int)pieceType] += f.Count;
                }
            }
